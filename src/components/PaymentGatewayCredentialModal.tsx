...
import type { PaymentGatewayCredential } from '@/lib/payment-gateway-credentials-api';
...
interface PaymentGatewayCredentialModalProps {
  isOpen: boolean;
  mode: 'create' | 'edit';
  credential: PaymentGatewayCredential | null;
  onClose: () => void;
...
const sandboxOptions = [
  { value: 'false', label: 'Production', description: 'Real credentials — moves real money' },
  { value: 'true', label: 'Sandbox', description: 'Test credentials — informational only for now, no balance separation yet' },
];

// Fields shown per provider — matches exactly what DuitkuProvider/MidtransSnapProvider
// read from `credentials` (see payment-service). Keep in sync with those files.
const PROVIDER_FIELDS: Record<string, { key: string; label: string; placeholder: string }[]> = {
  duitku: [
    { key: 'merchantCode', label: 'Merchant Code', placeholder: 'e.g. DS33970' },
    { key: 'apiKey', label: 'API Key', placeholder: 'Duitku API key' },
    { key: 'baseUrl', label: 'Base URL', placeholder: 'https://sandbox.duitku.com' },
    { key: 'callbackUrl', label: 'Callback URL', placeholder: 'https://payment.bagdja.com/payments/duitku/webhook' },
    { key: 'returnUrl', label: 'Return URL', placeholder: 'https://pay.bagdja.com/checkout/return' },
  ],
  midtrans: [
    { key: 'serverKey', label: 'Server Key', placeholder: 'Mid-server-...' },
    { key: 'clientKey', label: 'Client Key', placeholder: 'Mid-client-...' },
    { key: 'isProduction', label: "isProduction ('true' / 'false')", placeholder: 'false' },
  ],
};
      ...
      <div className="bg-[var(--bg-surface)] rounded-lg border border-[var(--border-default)] w-full max-w-lg max-h-[90vh] overflow-y-auto m-4">
        <div className="sticky top-0 z-10 bg-[var(--bg-surface)] border-b border-[var(--border-default)] px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">
            {mode === 'create' ? 'Add Gateway Credential' : 'Edit Gateway Credential'}
          </h2>
          <button
            type="button"
            onClick={onClose}
